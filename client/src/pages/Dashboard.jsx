...
export default function Dashboard() {
  ...
  const getParentForRepo = (repoOwner, repoName) => {
    const matched = forkFamilies.find((family) =>
      family.forks.some((f) => f.owner.toLowerCase() === repoOwner.toLowerCase() && f.repo.toLowerCase() === repoName.toLowerCase())
    );
    return matched ? matched.parent : null;
  };
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                  {filteredNormalRepos.map((repo) => (
                    <RepoCard key={`${repo.owner}/${repo.name}`} repo={repo} parent={getParentForRepo(repo.owner, repo.name)} />
                  ))}
                </div>
              </div>
            )}
