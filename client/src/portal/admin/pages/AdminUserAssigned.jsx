...
import { Users, Search, RefreshCw, Plus, X, Eye, EyeOff, Loader2 } from 'lucide-react';
import { fetchAdminRelations, createPersonalUser } from '../../../store/slices/authSlice.js';
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-(--text-secondary) font-medium mt-1">
            Create, manage, and audit all personal users linked to your administrator profile.
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          >
            <RefreshCw size={12} className={relationsLoading ? 'animate-spin' : ''} />
            Reload List
          </button>
            ...
            <table className="w-full text-left border-collapse text-xs">
              ...
              <tbody className="divide-y divide-(--border)/60">
                {filteredRelations.map((relation) => {
                  ...
                  return (
                    <tr key={relation.unique_id} className="text-(--text-primary)">
                      <td className="py-4">
                        <div className="font-extrabold">{user.name || 'N/A'}</div>
                        <div className="text-[10px] text-(--text-secondary) font-mono">{user.email || 'N/A'}</div>
                      </td>
                      <td className="py-4 font-mono font-bold text-(--primary)">{user.unique_id || 'N/A'}</td>
                      <td className="py-4">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-xs">
          <div className="relative w-full max-w-md p-6 rounded-3xl border border-(--border) bg-(--bg-primary) shadow-2xl space-y-6">
            ...
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-black text-(--text-primary)">Add Personal User</h3>
              <button
